...
YoloFeatureExtractor::YoloFeatureExtractor() {}

FeatureSample YoloFeatureExtractor::extractFromSequence(const std::vector<cv::Mat>& frames,
                                                         int label,
    ...
    if (frames.size() < 2 || yolo_boxes.size() != frames.size()) return sample;
    ...
    for (int t = 0; t < total_frames; ++t) {
        cv::Rect b = yolo_boxes[t];
        if (b.width > 5 && b.height > 5) {
            centers[t] = cv::Point2f(b.x + b.width / 2.0f, b.y + b.height / 2.0f);
            actor_present[t] = true;
        } else {
            centers[t] = cv::Point2f(img_w / 2.0f, img_h / 2.0f);
            actor_present[t] = false;
        }
    }

    // 3-frame moving average filter on YOLO box centers to suppress single-frame jitter
    ...
    for (int t = 0; t < total_frames; ++t) {
        ...
        if (actor_present[t]) {
            ...
            if (box.width > 5 && box.height > 5) {
                ...
                // 1. Horizontal centroid translation speed
    ...
    if (first_active != -1 && last_active > first_active) {
        float dx_net = std::abs(smooth_centers[last_active].x - smooth_centers[first_active].x);
